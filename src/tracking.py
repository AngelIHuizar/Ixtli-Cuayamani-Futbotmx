...
def rastrear_video(ruta_video,
    ...
    # Anotadores
    mask_ann  = sv.MaskAnnotator(color_lookup=sv.ColorLookup.TRACK, opacity=0.5)
    ...
    while idx < frame_fin:
        ...
        if writer is not None:
            ...
            if tracked.tracker_id is not None and len(tracked) > 0:
                if tracked.mask is not None:                     # máscaras de SAM
                    anot = mask_ann.annotate(anot, tracked)
            ...
            if balon is not None:
                cv2.circle(anot, (int(balon[0]), int(balon[1])), 13, (0, 0, 255), 3)
                cv2.putText(anot, "balon", (int(balon[0])+15, int(balon[1])),
